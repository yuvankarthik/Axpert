...
function AxCallSqlDataAPI(apiPublicKey, sqlParams = {}) {
    ...
    var jsonVal = "";
    $.ajax({
        url: top.window.location.href.toLowerCase().substring("0", top.window.location.href.toLowerCase().indexOf("/aspx/")) + '/AxPlugins/AxpertAPIs/aspx/AxpertAPIs.aspx/CallDataSourceAPI',
        type: 'POST',
        cache: false,
        async: false,
        data: JSON.stringify({
...
function AxSetValue(apiPublicKey, fieldName, dcNo, rowNo, value) {
    var jsonVal = "";
    $.ajax({
        url: top.window.location.href.toLowerCase().substring("0", top.window.location.href.toLowerCase().indexOf("/aspx/")) + '/AxPlugins/AxpertAPIs/aspx/AxpertAPIs.aspx/AxSetValue',
        type: 'POST',
        cache: false,
        async: false,
        data: JSON.stringify({
...
function AxCallSubmitDataAPI(apiPublicKey, recordId) {
    var jsonVal = "";
    $.ajax({
        url: top.window.location.href.toLowerCase().substring("0", top.window.location.href.toLowerCase().indexOf("/aspx/")) + '/AxPlugins/AxpertAPIs/aspx/AxpertAPIs.aspx/CallSubmitDataAPI',
        type: 'POST',
        cache: false,
        async: false,
        data: JSON.stringify({
